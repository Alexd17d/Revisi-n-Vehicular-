 ...
 * 
 * Esta función solicita al usguario un nombre de usuario y contraseña,
...
int usuarioExiste(const char *usuario) {
    FILE *file = fopen(ARCHIVO_USUARIOS, "a+"); // Abrir en modo lectura y escritura, creando el archivo si no existe
    if (file == NULL) {

...
#define ARCHIVO_COMPROBANTES "D:/Proyecto/matriculacion_vehicular-main/data/comprobantes.txt"

//Función para vehiculos
void listarVehiculosArchivo() {
    FILE *file = fopen("D:/Proyecto/matriculacion_vehicular-main/data/vehiculos.txt", "rb");
    if (!file) {
    ...
    printf("\n%-5s %-10s %-15s %-10s %-6s %-10s\n", 
           "N°", "Placa", "Cedula", "Tipo", "Anio", "Avaluo");
    printf("-------------------------------------------------------------\n");
...
void mostrarDatosVehiculo(const Vehiculo* v) {
    if (v == NULL) {
        printf("Error: Vehiculo no valido.\n");
        return;
    }
...
float verificarMultas(){
    ...
    do
    {
        printf("\n¿Tiene multas de transito? (1 = Si, 0 = No): ");
        ...
        if (tieneMultas == 1) 
        {
            do {
                limpiarPantalla();
                printf("\nSeleccione la infraccion cometida:\n");
                printf("Multas: $%.2f\n", multa);
                printf("1. Exceso de velocidad - $120.00\n");
                printf("2. No uso de cinturon - $80.00\n");
...
void generarComprobanteArchivo() {
    ...
    if (v.estaMatriculado != 1) {
        printf("El vehiculo no esta matriculado.\n");
        mensajeSalida();
        return;
    }
...
int revision() {
    ...
    int frenos = leerRespuestaSN("¿Los frenos estan operables?");
    int luces  = leerRespuestaSN("¿Las luces funcionan correctamente?");
    int gases  = leerRespuestaSN("¿Los gases estan dentro de los limites permitidos?");
    int llantas = leerRespuestaSN("¿Las llantas estan en buen estado?");

    if (frenos && luces && gases && llantas) {
        printf("Revision tecnica aprobada.\n");
        return 1;
    } else {
        printf("Revision tecnica fallida.\n");
        return 0;
    }
}
...
void procesoMatriculacionArchivo() {
    ...
    if (v.revisionesAnuales > 3) {
        printf("Numero maximo de revisiones alcanzadas.\n");
        fclose(file);
        mensajeSalida();
        return;
    }

    printf("Numero de revisiones: %d\n", v.revisionesAnuales);
    ...
    printf("\nProceso de matriculacion completado.\n");
    fclose(file);
    mensajeSalida();
}
...
void listarVehiculosMatriculadosArchivo() {
    ...
    if (encontrados == 0) {
        printf("       No hay vehiculos matriculados registrados.\n");
    }
 ...
 */
    ...
    do
    {
        printf("Ingrese la placa del vehiculo (AAA-1234):\n");
        ...
        if (validarPlaca(v.placa) != 1) {
            printf("Error: Formato de placa invalido. Debe ser AAA-1234.\n");
        }
    ...
    do
    {
        ...
        if (validarCedula(v.cedula) != 1) {
            printf("Error: La cedula debe tener 10 digitos validos y un codigo de provincia correcto (01-24).\n");
        }
    ...
    do {
        printf("Ingrese el avaluo del vehiculo (positivo > $1000):\n");
...
void registrarVehiculoArchivo() {
    ...
    if (placaExisteArchivo(v.placa)) {
        printf("Error: Ya existe un vehiculo registrado con la placa %s.\n", v.placa);
        mensajeSalida();
        return;
    }

    FILE *file = fopen("D:/Proyecto/matriculacion_vehicular-main/data/vehiculos.txt", "ab"); // modo append binario
    if (file == NULL) {
